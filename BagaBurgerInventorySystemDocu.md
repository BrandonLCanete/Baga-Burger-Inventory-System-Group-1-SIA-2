...
## A. Introduction
 ...
 The Baga Burger is a food service establishment that creates and sell burgers and beverages to local. Restaurants similar to baga burger mostly face difficulties when doing manual-tracking inventory, giving them a hard time and can lead to low efficiency in overall operations. According to Indeed Editorial Team, 2022, An inventory system is a software or process used to track, manage, and control the flow of goods, materials, or products in a business. It monitors stock levels, orders, sales and deliveries to ensure that the right amount of inventory is available at all times. The system will focus on Baga Burger's operations to make the business run effieciently.
 
By automating the inventory process, Baga Burger can ensure timely restocking, avoid shortages, and improve overall productivity, helping the business scale more effectively. Our system supports features like Real-Time Stock Tracking, it monitors inventory levels to ensure products are always available when needed. SAN (Stock Alerts and Notifications) which sends alerts when stock is low to prevent shortages. Order Management that tracks incoming stock and updates inventory when new items are received. Reporting and Analytics which Provides simple reports on stock levels, usage, and trends. Inventory Categorization the one that organizes products into categories for easy tracking.
